...
inputFilenameForPathwayMatching = 'maleVsFemaleGenotypesJustFDR0p5.xlsx';
%filename with just p-values for the addition to the excel
inputFilenameOfPValues = 'maleVsFemaleTTestLog2Genotypes.xlsx';
...
isPvalue = 1;
geneticLables = {'APC KRAS PTEN', 'APC KRAS', 'APC'};
% labels for each dataset used
cc = 1;
for i =1:length(geneticLables)
        comparisonLabelsGenetic{cc} = [geneticLables{i} ' male vs. female'];
        cc = cc+1;
end
